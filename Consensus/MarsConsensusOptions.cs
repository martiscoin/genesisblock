...
{
    /// <inheritdoc />
    public class MarsConsensusOptions : PosConsensusOptions
    {
        ...
        public MarsConsensusOptions(Network network)
        {
        ...
        private void Assert(bool condition, [CallerMemberName] string caller = "", [CallerLineNumber] int line = -1)
        {
            if (!condition)
            {
                throw new Exception($"{nameof(MarsConsensusOptions)} - assert failed! Caller {caller}, line: {line}.");
            }
        }
    }
}